...
サブスクの一覧・毎月/年間の合計金額・おすすめ解約日の計算・カレンダー表示・毎月の支出推移グラフをまとめて管理するシンプルなWebアプリです。スマホ・タブレット・PCのブラウザで使えます。
...
- **ホーム画面に追加する**: `manifest.json` と `icon-192.png`/`icon-512.png`/`apple-touch-icon.png` を同梱しているので、スマホのブラウザで「ホーム画面に追加」すると、独自のアイコンでアプリのように起動します。
